...
1. Containarizing the app and exposing on 8000 port 
    - here used gunicorn to maintain multiple workers to serve concurrent requests(Added gunicorn.conf.py)
...
4. Build/ pull the docker image 'pknashi/food-app:1.0' and using k8s/deploy.yaml run the containers.
